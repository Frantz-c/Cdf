...
void Cliarg_free_arguments(cliarg_t **args)
{
	...
	while (tmpArgs)
	{
		...
		else if (tmpArgs->_type == INT) free(tmpArgs->value);
		else if (tmpArgs->_type == BOOL) free(tmpArgs->value);
		tmp2 = tmpArgs;
...
int		Cliarg_get_int_value(cliarg_t *args, char name)
{
	cliarg_t *tmp   = NULL;
